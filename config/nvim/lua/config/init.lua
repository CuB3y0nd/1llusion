...
local defaults = {
	...
	defaults = {
		...
		-- config.options can't be configured here since that's loaded before 1llusion setup
...
---@param opts? 1llusionConfig
function M.setup(opts)
	options = vim.tbl_deep_extend("force", defaults, opts or {})
	if not M.has() then
		require("lazy.core.util").error(
			"**1llusion** needs **lazy.nvim** version "
				.. M.lazy_version
				.. " to work properly.\n"
...
function M.load(name)
	...
	local pattern = "1llusion" .. name:sub(1, 1):upper() .. name:sub(2)
